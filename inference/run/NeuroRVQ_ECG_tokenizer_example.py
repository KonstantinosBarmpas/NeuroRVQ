...
import torch
import os
from NeuroRVQ_ECG.NeuroRVQ import NeuroRVQTokenizer
...
from preprocessing.preprocessing_ecg_example import create_patches
...
def load_neurorqv_tokenizer(run_example=False, plot_results=False, verbose=False,
    ...
    if (run_example):
        x = np.load("./example_files/ecg_sample/ptb_xl_cut_benchmarking.npy")
        # Get one sample
        x = x[:1]
        ch_names = np.array([b'i', b'ii', b'iii', b'avr', b'avl', b'avf', b'v1', b'v2', b'v3', b'v4', b'v5', b'v6'])
        ...
        if (plot_results):
            process_and_plot(oringal_signal_std, reconstructed_signal_std, fs=200, mode='ECG')
