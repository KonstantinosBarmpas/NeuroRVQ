...
import torch
import os
from NeuroRVQ_PPG.NeuroRVQ import NeuroRVQTokenizer
from NeuroRVQ_PPG.NeuroRVQ_modules import get_encoder_decoder_params
from plotting.plotting_example import process_and_plot
from preprocessing.preprocessing_ppg_example import create_patches
from inference.modules.NeuroRVQ_PPG_tokenizer_inference_modules import ch_names_global, create_embedding_ix, check_model_eval_mode
...
def load_neurorqv_tokenizer(run_example=False, plot_results=False, verbose=False,
    ...
    config_stream = open("./flags/NeuroRVQ_PPG_v1.yml", 'r')
    ...
    if (run_example):
        x = np.load("./example_files/ppg_sample/example_ppg.npy")
        ch_names = np.array([b'ppg_c1'])
        ...
        if (plot_results):
            process_and_plot(oringal_signal_std, reconstructed_signal_std, fs=100, mode='PPG')
